...
class Article_model extends CI_Model{
		...
		switch($contentType){
			...
			default:
				...
				if(count($article) > 0){
					...
					if($this->archive_db->table_exists($tblname)){
						$article = $this->archive_db->query("SELECT content_id , section_id , section_name , publish_start_date , last_updated_on , title , url , summary_html , article_page_content_html , article_page_image_path , article_page_image_title , article_page_image_alt , tags , agency_name , author_name , author_image_path , author_image_title , author_image_alt , no_indexed , no_follow , meta_Title , meta_description , amp_status  FROM ".$tblname." WHERE status='P' AND publish_start_date < NOW() AND content_id='".$contentID."'")->row_array();
						$this->memcached_library->add($CacheID,$article);
						return $article;
					}
				}
	...
	public function menuDetails(){
		$CacheID = 'menulist';
		if(!$this->memcached_library->get($CacheID) && $this->memcached_library->get($CacheID) == ''){
			$menus = $this->live_db->query("SELECT Section_id , URLSectionStructure , Sectionname  FROM sectionmaster WHERE Status=1 AND ParentSectionID IS NULL AND IsSubSection=0 AND IsSeperateWebsite=0 AND MenuVisibility=1 ORDER BY DisplayOrder ASC LIMIT 9")->result_array();
	...
	public function trendingStories(){
		...
		$fromDate1 = date("Y-m-d", strtotime("-1 week")).' 00:00:00';
		$endDate2 = date('Y-m-d').' 23:59:59';
		$CacheID = "SELECT article.url , article.tags , tag.tag_name FROM article AS article INNER JOIN seo_tags AS tag ON article.content_id=tag.content_id WHERE article.content_id IN(SELECT content_id FROM content_hit_history WHERE content_type=1 AND created_on BETWEEN '".$fromDate."' AND '".$endDate."' ORDER BY hits DESC) AND article.status='P' AND article.amp_status=1 AND tag.tag_name!='' LIMIT 5";
		if(!$this->memcached_library->get($CacheID) && $this->memcached_library->get($CacheID) == '') {
			$query = $this->live_db->query($CacheID);
			$response['trending_today'] = $query->result_array();	
			$this->memcached_library->add($CacheID,$result);
		} else {
			$response['trending_today'] = $this->memcached_library->get($CacheID);
		}
		$CacheID1 = "SELECT article.url , article.tags , tag.tag_name FROM article AS article INNER JOIN seo_tags AS tag ON article.content_id=tag.content_id WHERE article.content_id IN(SELECT content_id FROM content_hit_history WHERE content_type=1 AND created_on BETWEEN '".$fromDate1."' AND '".$endDate2."' ORDER BY hits DESC) AND article.status='P'  AND article.amp_status=1 AND tag.tag_name!='' LIMIT 5";
		if(!$this->memcached_library->get($CacheID1) && $this->memcached_library->get($CacheID1) == '') {
			$query = $this->live_db->query($CacheID1);
			$response['trending_week'] = $query->result_array();	
			$this->memcached_library->add($CacheID1,$result);
		} else {
			$response['trending_week'] = $this->memcached_library->get($CacheID1);
		}
		$CacheID2 = "SELECT article.url , article.tags , tag.tag_name FROM article AS article INNER JOIN seo_tags AS tag ON article.content_id=tag.content_id WHERE article.status='P' AND article.amp_status=1 AND article.tags!='' AND article.publish_start_date < NOW() ORDER BY article.publish_start_date DESC LIMIT 5";
		if(!$this->memcached_library->get($CacheID2) && $this->memcached_library->get($CacheID2) == '') {
			$query = $this->live_db->query($CacheID2);
			$response['latest'] = $query->result_array();	
			$this->memcached_library->add($CacheID2,$result);
		} else {
			$response['latest'] = $this->memcached_library->get($CacheID2);
		}
		return $response;
	}
}
