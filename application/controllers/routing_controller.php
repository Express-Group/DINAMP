...
class routing_controller extends CI_Controller{
	...
	public function section($sectionname){
		...
		$section =  $this->live_db->query("SELECT URLSectionStructure FROM sectionmaster WHERE (Sectionname='".$sectionname."' OR URLSectionName ='".$sectionname."') LIMIT 1")->row_array();
		if(count($section) > 0){
	...
	public function article($contentID ,$contentType='article'){
		...
		$splitdata = explode('-',$contentID);
		$contentID = $splitdata[1];
		$this->live_db = $this->load->database('live_db' , TRUE);
